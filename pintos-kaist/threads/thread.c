...
thread_yield(void) {
	...
	do_schedule (THREAD_READY);  // 여기서 status 처리함
	intr_set_level (old_level);
}
...
thread_sleep (int64_t ticks) {
	...
	if (curr != idle_thread){
		...
		list_insert_ordered (&sleep_list, &curr->elem, cmp_waketick, 0);
...
thread_wakeup (int64_t ticks){
	...
	}while (e != list_rbegin(&sleep_list));
	
}

static bool 
cmp_waketick(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED){
...
thread_set_priority (int new_priority) {
	...
	if(list_empty(&curr->donations)){
		...
		curr->priority = new_priority;
		curr->original_priority = new_priority;
		for(int i = 0; i<list_size(&curr->donations); i++){
